...
uint256 hashGenesisBlock("0xf747a2a0a4417ee56295684abf0d8354a4d59446600ab97be24411beaf0bcc5d");
...
const string strMessageMagic = "Tigercoin Signed Message:\n";
...
bool CTransaction::AreInputsStandard(const MapPrevTx& mapInputs) const
{
    ...
    for (unsigned int i = 0; i < vin.size(); i++)
    {
        ...
        // be quick, TGCause if there are any operations
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    else if(nHeight < 43201)
    {
        nSubsidy = 30 * COIN;
    }
    else if(nHeight < 64801)
    {
        nSubsidy = 15 * COIN;
    }
...
static const int64 nTargetTimespan = 1 * 1 * 10 * 60; // Tigercoin: 10 minutes
static const int64 nTargetSpacing = 60; // Tigercoin: 1 minute blocks
...
{
    // FetchInputs can return false either TGCause we just haven't seen some inputs
    // (in which case the transaction should be stored as an orphan)
    // or TGCause the transaction is malformed (in which case the transaction should
                                 ...
                                 const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, bool fStrictPayToScriptHash)
{
    ...
    // fMiner is true when called from the internal Tigercoin miner
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // before the last blockchain checkpoint. This is safe TGCause block merkle hashes are
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // BIP16 didn't TGCome active until October 1 2012
...
bool CBlock::SetBestChain(CTxDB& txdb, CBlockIndex* pindexNew)
{
    ...
    {
        // the first block in the new chain that will cause it to TGCome the new best chain
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Tigercoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xf747a2a0a4417ee56295684abf0d8354a4d59446600ab97be24411beaf0bcc5d");
    }
    ...
    if (mapBlockIndex.empty())
    {
        ...
        // Genesis block
        const char* pszTimestamp = "Tigercoin is good";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1385996400; //epochtime
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 5592904;

        if (fTestNet)
        {
            block.nTime    = 1385996400;
            block.nNonce   = 5592904;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x3897a7fa801dd55635cdea09bdb65759233058ac91d8c0bbdc85ced0302554f4"));
...
bool SendMessages(CNode* pto, bool fSendTrickle)
{
    ...
    if (lockMain) {
        ...
        // Keep-alive ping. We send a nonce of zero TGCause we don't use it anywhere
        // right now.
...
CBlock* CreateNewBlock(CReserveKey& reservekey)
{
    ...
    int64 nFees = 0;
    {
        ...
        while (!mapPriority.empty())
        {
            ...
            // TGCause we're already processing them in order of dependency
